use std::{fs, collections::{HashSet, HashMap}, cmp::{max, min}};
...
fn get_adj(loc : &Loc, dir : char) -> Vec<Loc> {
    match dir {
        ...
        'E' => {
            vec![(loc.0 - 1, loc.1 + 1), (loc.0, loc.1 + 1), (loc.0 + 1, loc.1 + 1)]
        }
        _=> {
            vec![(loc.0 - 1, loc.1 - 1), (loc.0, loc.1 - 1), (loc.0 + 1, loc.1 - 1)]
        }
...
fn main() {
        ...
        for e in &elves {
            ...
            for dir in dirList {
                ...
                if adj.iter().all(|l| !elves.contains(l)) {
                    ...
                    proposal_to_count.entry(adj[1]).and_modify(|v| *v += 1).or_insert(1);
                    done = false;
                    break;
                }
            }
    ...
    let (start, end) = bounds(&elves);
    let area = (end.0 - start.0 + 1) * (end.1 - start.1 + 1);

    println!("Part1: {}", area - elves.len() as i32);
