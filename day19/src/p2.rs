...
fn line_parse(input: &str) -> IResult<&str, Blueprint> {
    ...
    let max_ore = costs.iter().map(|c| c[ORE]).max().unwrap();
    let max_clay = costs[OBSIDIAN][CLAY];
    let bluep = Blueprint {
        costs,
        max_ore,
        max_clay,
...
fn gen_states(s: &State, bp: &Blueprint) -> Vec<State> {
    ...
    if s.resources[OBSIDIAN] >= bp.costs[GEODE][OBSIDIAN] && s.resources[ORE] >= bp.costs[GEODE][ORE]
    {
    ...
    if s.time >= 4 && s.bots[ORE] < bp.max_ore && s.resources[ORE] >= bp.costs[ORE][ORE] && need_no
    {
        let mut bot = no_bot.clone();
        bot.bots[ORE] += 1;
...
fn max_fut_geodes(s: &State, bp: &Blueprint) -> i32 {
...
fn main() {
    ...
    for bp in &bprints[0..3] {
        ...
        while opts.len() > 0 {
            ...
            states_considerd += 1;
            if s.time < 9 && max_fut_geodes(&s, &bp) < max_geodes {
                //Discard
            } else {
                if s.time == 0 {
